...
class userLogin:
    ...
    def login(self):
        ...
        if self.result:
            ...
            if status == "inactive":
                tkinter.messagebox.showwarning("Warning", "Student is inactive try again later")
            else:
                tkinter.messagebox.showinfo("Success", "Student login successfully")
                self.root.destroy()
                user_dashboard.StudentDashboard(self.result)
                # Place your navigation call here (e.g., open dashboard window)
        else:
