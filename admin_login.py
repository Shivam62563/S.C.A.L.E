...
class AdminLogin:
    ...
    def login(self):
        ...
        if self.result:
            ...
            if status == "inactive":
                tkinter.messagebox.showwarning("Warning", "admin is inactive try again later")
            else:
                tkinter.messagebox.showinfo("Success", "admin login successfully")
                self.root.destroy()
                adminDashboard.admindashboard(self.result)
